...
import { getMetadataArtistName } from '@/utils/artists';
...
const cacheKeyForSong = (song: Song): string => {
  return `${slugify(getMetadataArtistName(song.artist, song.title) || 'unknown-artist')}--${slugify(song.title || song.id)}`;
};

type SongGenreLoadResult =
